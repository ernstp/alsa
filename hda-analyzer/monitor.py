...
class Monitor(gtk.Window):
    ...
    def generate_sound(self):
        self.set_status('Playing sound #%s on channel %s...' % (self.generate_num, CHANNELS[self.channel]))
        self.generate_num += 1
        channels = 2
        if self.channel >= 6:
            channels = 8
            ...
            channels = 4
        self.cmd = ["speaker-test", "-D", self.device, "-c", str(channels), "-s", str(self.channel + 1)]
        p = Popen(self.cmd, shell=False, bufsize=0, stdin=None, stdout=PIPE, stderr=PIPE, close_fds=True)
        for fd in [p.stdout.fileno(), p.stderr.fileno()]:
            set_fd_nonblocking(fd)
        self.generate_p = p
        self.generate_stdout_id = gobject.io_add_watch(p.stdout, gobject.IO_IN | gobject.IO_HUP | gobject.IO_NVAL,
                                                       self.generate_io_stdout)
        self.generate_stderr_id = gobject.io_add_watch(p.stderr, gobject.IO_IN | gobject.IO_HUP | gobject.IO_NVAL,
                                                       self.generate_io_stderr)
    ...
    def record_sound(self):
        ...
        self.cmd = ["arecord", "-D", self.device, "-f", "dat", "-c", str(self.channels), "-t", "raw", "-vvv",
                    "/dev/null"]
        p = Popen(self.cmd, shell=False, bufsize=0, stdin=None, stdout=PIPE, stderr=PIPE, close_fds=True)
        for fd in [p.stdout.fileno(), p.stderr.fileno()]:
            set_fd_nonblocking(fd)
        self.record_p = p
        self.record_stdout_id = gobject.io_add_watch(p.stdout, gobject.IO_IN | gobject.IO_HUP | gobject.IO_NVAL,
                                                     self.record_io_stdout)
        self.record_stderr_id = gobject.io_add_watch(p.stderr, gobject.IO_IN | gobject.IO_HUP | gobject.IO_NVAL,
                                                     self.record_io_stderr)
