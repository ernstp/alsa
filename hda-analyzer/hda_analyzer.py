...
from hda_codec import HDACodec, HDA_card_list, HDA_Exporter_pyscript, EAPDBTL_BITS, PIN_WIDGET_CONTROL_BITS, \
    PIN_WIDGET_CONTROL_VREF, DIG1_BITS, GPIO_IDS, HDA_INPUT, HDA_OUTPUT
...
class HDAAnalyzer(Gtk.Window):
    ...
    def simple_dialog(self, type, msg):
        dialog = Gtk.MessageDialog(self, Gtk.DialogFlags.MODAL | Gtk.DialogFlags.DESTROY_WITH_PARENT, type,
...
"""
    ...
    def __diff_clicked(self, button):
        if not self.codec:
            ...
            return
        dialog = Gtk.Dialog('Diff', self, Gtk.DialogFlags.MODAL | Gtk.DialogFlags.DESTROY_WITH_PARENT,
                            (Gtk.STOCK_OK, Gtk.ResponseType.OK))
        text_view = Gtk.TextView()
        text_view.set_border_width(4)
    ...
    def __export_clicked(self, button):
        ...
        dialog = Gtk.Dialog(exporter.title(), self, Gtk.DialogFlags.MODAL | Gtk.DialogFlags.DESTROY_WITH_PARENT,
        ...
        if r == Gtk.ResponseType.YES:
            sdialog = Gtk.FileChooserDialog('Save %s as...' % exporter.stitle(), self, Gtk.FileChooserAction.SAVE, (
    ...
    def __create_treeview(self):
        model = Gtk.TreeStore(GObject.TYPE_STRING, GObject.TYPE_INT, GObject.TYPE_INT, GObject.TYPE_INT,
        ...
        for card in CODEC_TREE:
            ...
            model.set(iter, TITLE_COLUMN, 'card-%s' % card, CARD_COLUMN, card, CODEC_COLUMN, -1, NODE_COLUMN, -1,
                      ITALIC_COLUMN, False)
            for codec in CODEC_TREE[card]:
                citer = model.append(iter)
                codec = CODEC_TREE[card][codec]
                model.set(citer, TITLE_COLUMN, 'codec-%s' % codec.device, CARD_COLUMN, card, CODEC_COLUMN, codec.device,
                          NODE_COLUMN, -1, ITALIC_COLUMN, False)
                for nid in codec.nodes:
                    viter = model.append(citer)
                    node = codec.get_node(nid)
                    model.set(viter, TITLE_COLUMN, 'Node[0x%02x] %s' % (nid, node.wtype_id), CARD_COLUMN, card,
                              CODEC_COLUMN, codec.device, NODE_COLUMN, nid, ITALIC_COLUMN, False)
                    nid += 1

        cell = Gtk.CellRendererText()
        cell.set_property('style', Pango.Style.ITALIC)

        column = Gtk.TreeViewColumn('Nodes', cell, text=TITLE_COLUMN, style_set=ITALIC_COLUMN)
