...
from hda_codec import EAPDBTL_BITS, PIN_WIDGET_CONTROL_BITS, PIN_WIDGET_CONTROL_VREF, DIG1_BITS, GPIO_IDS, HDA_INPUT, \
    HDA_OUTPUT
...
class Node:
    ...
    def expose(self, cr, event, graph):
        ...
        cr.select_font_face("Misc Fixed", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_NORMAL)
        cr.set_font_size(14)
    ...
    def in_area(self, x, y):
        if x >= self.myarea[0] and y >= self.myarea[1] and x < self.myarea[0] + self.myarea[2] and y < self.myarea[1] + \
                self.myarea[3]:
...
class Route:
    ...
    def longdesc(self):
        ...
        src.wtype_id.replace('_', '-'), src.nid, dst.wtype_id.replace('_', '-'), dst.nid)
    ...
    def statusdesc(self):
        ...
        res = niceprint("SRC " + src, vals[0]) + ' -> ' + niceprint("DST " + dst, vals[1])
        return res
        ...
        for p in possible:
            ...
            for route in routes:
                ...
                for line in route.lines:
                    if check_dot(line[0], line[1], p) or check_dot(line[2], line[3], p) or check_dot(p[0], p[1],
                                                                                                     line) or check_dot(
                        p[2], p[3], line):
                        # print "Found1", p
                        found = True
    ...
    def analyze_routes(self, routes, nodes):
        ...
        for j in xrange:
            possible.append([startx + j, posy + height + 5, startx + j, dposy + height + 5])
    ...
    def finish(self, routes, nodes):
        ...
        if tryit >= 0:
            ...
            print("[1] displaced route 0x%x->0x%x %s %s" % (
            self.src.node.nid, self.dst.node.nid, repr(self.lines[-1]), repr(sel)))
            res = False
        ...
        if tryit >= 0:
            ...
            print("[2] displaced route 0x%x->0x%x %s %s" % (
            self.src.node.nid, self.dst.node.nid, repr(self.lines[-1]), repr(sel)))
            res = False
...
class CodecGraphLayout(Gtk.Layout):
    ...
    def __init__(self, adj1, adj2, codec, mytitle, statusbar):
        ...
        self.add_events(
            Gdk.EventMask.EXPOSURE_MASK | Gdk.EventMask.POINTER_MOTION_MASK | Gdk.EventMask.BUTTON_PRESS_MASK | Gdk.EventMask.BUTTON_RELEASE_MASK | Gdk.EventMask.LEAVE_NOTIFY_MASK | Gdk.EventMask.SCROLL_MASK)
